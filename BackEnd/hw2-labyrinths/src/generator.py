...
class Generator:
    ...
    def generate(width: int, height: int, alg: str, terrain: None | int = None) -> Maze:
        Generator._validate_size(width, height)
            ...
            for x in range(maze.width):
                if maze.get_cell(x, y).type == CellType.SPACE:
                    if random.randint(1, 10) < Generator.TERRAIN_CHANCE:
                        maze.set_cell(x, y, random.choice([CellType.SAND, CellType.SWAMP, CellType.COIN]))

    @staticmethod
    def _validate_size(width, height):
        if width <= 0 or height <= 0:
            raise ValueError(f"Заданы неверные размеры ширина: {width}, высота: {height}")
