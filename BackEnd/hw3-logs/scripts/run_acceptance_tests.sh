...
tag="${CI_PROJECT_NAME:-local}-${CI_COMMIT_SHA:-latest}"
...
function runTest {
 ...
 echo "Test [№${testNumber}][${test_type}]: ${description}; expected exit code: ${expected_code}; args: $@;"
 docker run --rm -v $(pwd)/scripts/data:/tmp/data logs-app:$tag "$@"

 exit_code=$?
 assertExitCode $expected_code $exit_code
...
runTest "positive" "multiple files" 0 \
